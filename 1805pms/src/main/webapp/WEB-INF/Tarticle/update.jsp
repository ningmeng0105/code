...
<script type="text/javascript">
	...
	var CHKCONTENT="^[\\S]+$";
	//作者姓名
	var CHKAUTHOR="^([\u4e00-\u9fa5]{2,5}|[a-zA-Z]{3,16})$";
	...
	function chkauthor(){
		//获取作者姓名
		var auEle=document.getElementById("author");
		var author=auEle.value;
		//定义匹配作者姓名的正则表达式
		...
		if(reg.test(author)){//表示输入正确
			...
			spanEle.innerHTML="作者姓名必须为汉字或英文，且不能低于2位";
			spanEle.style.color="red";
			//清空文本框
			auEle.value="";
			//重新聚焦
			...
			<tbody id="tab_1">
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<input type="text" id="tittle" value="${article.tittle }" class="required" name="tittle" maxlength="10"  onblur="chktittle()"/>
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<textarea id="content" name="content" rows="15" cols="150" onblur="chkcontent()">${article.content }</textarea><br />
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<input type="text" id="author" class="required" value="${article.author }" name="author" maxlength="10" onblur="chkauthor()"/>
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						创建时间:</td><td width="80%" class="pn-fcontent">
							<!--引时间控件-->
						<input type="text"  name="crtime" maxlength="80"  value="${article.crtime }" class="Wdate" onclick="WdatePicker()" />
					</td>
				</tr>
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<select name="channel">
							...
							<!-- 显示所属栏目 -->
								<c:if test="${article.channel==channel.cname }">
									<option value="${channel.cname }"  selected="selected">${channel.cname }</option>
								</c:if>
								<!-- 显示其他栏目 -->
								<c:if test="${article.channel!=channel.cname }">
									<option value="${channel.cname }" >${channel.cname }</option>
								</c:if>
							</c:forEach>
						</select>
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<c:if test="${article.isremand==1 }">
							<input type="radio"  value="1" name="isremand"  checked="checked"/>推荐
							<input type="radio" value="0"  name="isremand" />不推荐
						</c:if>
						<c:if test="${article.isremand==0 }">
						<input type="radio" value="1"  name="isremand"  "/>推荐
							<input type="radio" value="0"  name="isremand" checked="checked"/>不推荐
						</c:if>
					</td>
				</tr>
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<c:if test="${article.ishot==1 }">
							<input type="radio" value="1" name="ishot"   checked="checked"/>是热点
							<input type="radio" value="0" name="ishot" /> 不是热点
						</c:if>
						<c:if test="${article.ishot==0 }">
							<input type="radio" value="1" name="ishot"  />是热点
							<input type="radio" value="0" name="ishot"  checked="checked"/> 不是热点
						</c:if>
					</td>
				</tr>
