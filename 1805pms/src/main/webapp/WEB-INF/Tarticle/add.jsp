...
/*	//验证标题
	...
	*/
	...
	function chktittle(){
		...
		//定义匹配标题的正则表达式
		var reg=new RegExp(CHKTITTLE);
		if(reg.test(tittle)){//正确
			if(chkExistTittle(tittle)){
				return true;
			}else{
				return false;	
			}
		}else{
			$("#resultTittle").html("标题不能为空");
			$("#resultTittle").css("color","red");
			//清空文本框
			$("#tittle").val("");
			//重新聚焦
			$("#tittle").focus();
	...
	//验证标题名是否唯一
	function chkExistTittle(tittle){
		var flag=false;
		$.ajax({
			//请求路径
			url:'chkarticle.do',
			...
			success:function(flag){
				if(flag=='true'){//没有重复
					$("#resultTittle").html("✔");
					$("#resultTittle").css("color","green");
					flag=true;
				}else{
					$("#resultTittle").html("此标题已存在");
					$("#resultTittle").css("color","red");
					//清空文本框
					$("#tittle").val("");
					//重新聚焦
			...
			<tbody id="tab_1">
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<input type="text" id="tittle" class="required" name="tittle" maxlength="10" onblur="chktittle()"/>
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						内容:</td><td width="80%" class="pn-fcontent">
							<textarea id="content" name="content" rows="15" cols="150" onblur="chkcontent()"></textarea><br />
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<input type="text" id="author" class="required" name="author" maxlength="10" onblur="chkauthor()"/>
